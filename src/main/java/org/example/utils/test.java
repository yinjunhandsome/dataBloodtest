...
public class test {
    ...
    public static Blood parser(LogicalPlan plan) {
        ...
        if (plan instanceof SubqueryAlias) {
            ...
            // 递归解析别名对应的子计划
            Blood parser = parser(aliasPlan.child());
            parser.setCurrentTableName(alias);
            return parser;
        }
        ...
        if (plan instanceof Project) {
            ...
            // 解析Project中的所有字段
            Map<String, String> stringStringMap = parseProjectFields(projectPlan);
            // 递归解析Project的子节点（Filter/UnresolvedRelation等）
            Blood blood=parser(projectPlan.child());
            Map<String, String> flatMap = handleColumn(stringStringMap, blood);
            blood.setFieldMap(flatMap);
            return blood;
            // 给字段节点关联源表/上游字段
        ...
        if (plan instanceof LogicalRelation) {
            ...
            return Blood.builder().currentTableName(TableName).sourceTableName(TableName).sources(attrNameList).dbName(dbName).build();
        }
...
//    }
    ...
    private static Map<String, String> handleColumn(Map<String, String> map, Blood blood) {
        ...
        map.entrySet().forEach(e->{
            if(sources.contains(e.getKey())||sources.contains(e.getKey().replace(blood.currentTableName+".",""))) {
                flat.put(e.getValue(),e.getKey().replace(blood.currentTableName+".",""));
            }
            else {
